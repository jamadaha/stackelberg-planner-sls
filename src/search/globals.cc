...
int calculate_fix_actions_plan_cost(
    const vector<const GlobalOperator *> &plan) {
  int plan_cost = 0;
...
void save_plan(const vector<const GlobalOperator *> &plan, int) {
  ...
  /*   ofstream outfile;
     ...
     endl; outfile << "(" << plan[i]->get_name() << ")" << endl;
     }
...
bool is_unit_cost() { return g_min_action_cost == 1 && g_max_action_cost == 1; }

bool has_axioms() { return !g_axioms.empty(); }
