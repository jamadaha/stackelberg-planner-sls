...
Combiner::Combiner(const std::size_t &&min_parameters,
...
void Combiner::Init(std::shared_ptr<symbolic::SymVariables> vars,
                    const World &world) {
  this->vars = vars;
      ...
      for (auto permutation : Cartesian(predicate_params, param_count + i)) {
        ...
        for (const auto &p : permutation)
          if (p >= param_count)
            novel = true;
        if (novel)
          this->expansion_literals[i].push_back(
              Literal(t, std::move(permutation)));
      }
    }
  }
}

void Combiner::Combine(const World &world, const BDD &valid, const BDD &invalid,
...
std::vector<Literal> Combiner::GenerateRoot(const World &world,
...
void Combiner::ExploreDAG(const World &world,
                          const std::vector<Literal> &literals,
                          std::vector<std::size_t> &comb, const BDD &valid,
                          const BDD &invalid,
                          std::function<void(Combination)> f) {
  for (std::size_t i = comb.empty() ? 0 : comb.back() + 1; i < literals.size();
       i++) {
    comb.push_back(i);
...
BDD Combiner::Reduce(const World &world, const std::vector<Literal> &literals,
                     const std::vector<std::size_t> &comb, const BDD &bdd) {
...
void Combiner::Expand(const World &, const std::vector<Literal> &,
                      const std::vector<std::size_t> &, const BDD &,
                      const BDD &, std::function<void(Combination)>) {
  // for (std::size_t i = 1; i <= this->max_parameters; i++) {
  ...
  //         }
  //         for (const auto &e_comb : Comb(e_literals.size(), 1,
  //         this->max_precondition_size - p_comb.size())) {
  //             std::vector<std::size_t> comb = p_comb;
  //             for (const auto c : e_comb) comb.push_back(c + p_comb.size());
  //             const BDD c_valid = Reduce(world, instantiations, literals,
  //             comb, valid); const BDD c_invalid = Reduce(world,
  //             instantiations, literals, comb, invalid); const BDD
  //             c_applicable = c_valid | c_invalid; if
  //             (vars->numStates(c_applicable) != 0) {
  //                 std::vector<Literal> c_literals;
  //                 for (const auto &i : comb)
  //                     c_literals.push_back(literals[i]);
  //                 f(Combination(
