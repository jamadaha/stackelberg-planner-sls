...
int StackelbergBounded::compute_pareto_frontier(const GlobalState &state, vector<const GlobalOperator *> &leader_ops_sequence,
...
void StackelbergBounded::iterate_applicable_ops(const
                                               vector<const GlobalOperator *> &applicable_ops,
...
SearchEngine *_parse_bounded(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        return new stackelberg::StackelbergBounded(opts);
    }
    return NULL;
}
